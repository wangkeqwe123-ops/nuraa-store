...
export function GiftSection({locale,sections}:{locale:Locale;sections:HomepageContentSection[]}) { const {gifts}=getBrandContent(locale); return <section id="gifts" className="scroll-mt-24 bg-white px-6 py-32 lg:px-10 lg:py-40"><div className="mx-auto max-w-[1500px]"><Reveal>
  ...
  <div className="grid gap-px border border-black/20 bg-black/20 sm:grid-cols-2 lg:grid-cols-4">{sections.map((section,index)=>{const item=gifts.items[index];const content=localizeHomepageSection(section,locale);const title=locale==="ar"&&!section.titleAr&&item?item[0]:content.title;const subtitle=locale==="ar"&&!section.subtitleAr&&item?item[1]:content.subtitle;const link=content.ctaLink.replace(/^\/en(?=\/)/,`/${locale}`)||`/${locale}/products`;return <Link href={link} key={section.id} className="group bg-white p-4"><div className="relative aspect-[4/5] overflow-hidden bg-[#EEEAE3]"><HomepageMedia section={section} alt={title} sizes="(max-width:639px) 100vw,(max-width:1023px) 50vw,25vw" className="image-reveal transition duration-[1400ms] ease-out group-hover:scale-[1.035]"/><div className="absolute inset-0 bg-gradient-to-t from-black/55 via-transparent to-transparent"/><h3 className="font-display absolute bottom-6 start-6 text-4xl font-medium text-white">{title}</h3></div><div className="flex items-start justify-between gap-4 px-1 py-5"><p className="text-base leading-7 text-black/60">{subtitle}</p><ArrowUpRight className="mt-1 shrink-0 text-black transition-transform duration-500 group-hover:translate-x-1 group-hover:-translate-y-1" size={18}/></div></Link>})}</div>
