...
export function CheckoutPage({ locale }: { locale: Locale }) {
  ...
  if(!cart)return <div className="grid min-h-[60vh] place-items-center bg-[#fbfaf6]"><Loader2 className="animate-spin text-[#1f342b]"/></div>;
  if(!cart.items.length)return <main className="grid min-h-[60vh] place-items-center bg-[#fbfaf6] text-center"><div><h1 className="font-display text-5xl text-[#1f342b]">{copy.empty}</h1><Link className="mt-8 inline-flex min-h-11 items-center border-b border-[#1f342b]" href={`/${locale}/cart`}>{copy.back}</Link></div></main>;
  const shipping=cart.subtotal>=250?0:25;
  return <main className="bg-[#f3f0e9] text-[#1b1d1a]"><section className="mx-auto max-w-6xl px-5 py-14 md:px-10 md:py-20"><p className="text-xs font-semibold uppercase tracking-[.2em] text-[#7b4d35]">{copy.eyebrow}</p><h1 className="font-display mt-4 text-6xl text-[#1f342b] md:text-8xl">{copy.title}</h1>
