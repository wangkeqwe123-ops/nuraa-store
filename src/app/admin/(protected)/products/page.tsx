...
export default async function AdminProducts({searchParams}:{searchParams:Promise<{q?:string;status?:string}>}){
  ...
  return <>
    <PageHeader eyebrow="Catalog" title="Products" description={`${products.length} products in the current view.`} actions={<Button nativeButton={false} render={<Link href="/admin/products/new"/>} className="bg-[#12372a] hover:bg-[#0d2b21]"><Plus/>Add product</Button>}/>
    ...
    <Card className="mt-5 border-[#12372a]/10 shadow-none"><CardContent className="p-0"><form className="flex flex-col gap-3 border-b p-4 sm:flex-row"><div className="relative flex-1"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground"/><Input name="q" defaultValue={q} placeholder="Search name or SKU…" className="pl-9"/></div><select name="status" defaultValue={status??""} className="h-9 rounded-lg border bg-white px-3 text-sm"><option value="">All status</option><option value="ACTIVE">Active</option><option value="DRAFT">Draft</option><option value="ARCHIVED">Archived</option></select><Button variant="outline" type="submit">Filter</Button></form>
