...
export async function saveProduct(id: string | undefined, _previousState: ProductFormState, formData: FormData): Promise<ProductFormState> {
  ...
  try {
    ...
    if (!productId) {
      const product = await db.product.create({
        data: {
          ...
          translations: {
            create: [
              { locale: "EN", name: data.name_en, description: data.name_en, benefits: [], fragranceNotes: [], topNotes: [], heartNotes: [], baseNotes: [], usageOccasions: [] },
              { locale: "AR", name: data.name_ar, description: data.name_ar, benefits: [], fragranceNotes: [], topNotes: [], heartNotes: [], baseNotes: [], usageOccasions: [] },
            ],
          },
        },
      });
    ...
    const replacedIds = new Set([
      ...
      ...(video ? currentMedia.filter((item) => item.type === "VIDEO").map((item) => item.id) : []),
    ]);
    ...
    await db.$transaction(async (tx) => {
      ...
      await tx.productTranslation.upsert({
        ...
        create: { productId, locale: "EN", name: data.name_en, description: data.name_en, benefits: [], fragranceNotes: [], topNotes: [], heartNotes: [], baseNotes: [], usageOccasions: [] },
      });
      await tx.productTranslation.upsert({
        where: { productId_locale: { productId, locale: "AR" } },
        update: { name: data.name_ar },
        create: { productId, locale: "AR", name: data.name_ar, description: data.name_ar, benefits: [], fragranceNotes: [], topNotes: [], heartNotes: [], baseNotes: [], usageOccasions: [] },
      });
