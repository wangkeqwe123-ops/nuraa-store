...
const schema=z.object({eventType:z.enum(["PAGE_VIEW","PRODUCT_VIEW","ADD_TO_CART","CHECKOUT_START","WHATSAPP_CLICK"]),visitorId:z.string().uuid(),sessionId:z.string().uuid(),productId:z.string().optional(),productSlug:z.string().max(200).optional(),source:z.string().trim().max(80).optional(),path:z.string().max(500).optional(),referrer:z.string().max(1000).nullable().optional(),utmSource:z.string().max(100).nullable().optional(),utmMedium:z.string().max(100).nullable().optional(),utmCampaign:z.string().max(200).nullable().optional(),utmContent:z.string().max(200).nullable().optional()});
...
export async function POST(request:Request){
  ...
  await db.$transaction(async tx=>{
    await tx.analyticsEvent.create({data:{eventType:d.eventType,visitorId:d.visitorId,sessionId:d.sessionId,productId,path:d.path,referrer:d.referrer,trafficSourceId:traffic.id,metadata:d.source?{source:d.source}:undefined}});
